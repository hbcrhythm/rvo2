...
absSq(Vector2) ->
	rvo2_vector2:mult(Vector2, Vector2).
...
distSqPointLineSegment(Vector1, Vector2, Vector3) ->
	R = rvo2_vector2:mult(rvo2_vector2:sub(Vector3, Vector1), rvo2_vector2:sub(Vector2, Vector1)) / absSq(rvo2_vector2:sub(Vector2, Vector1)),
	if
		R < 0.0 ->
			absSq(rvo2_vector2:sub(Vector3, Vector1));
		R > 1.0 ->
			absSq(rvo2_vector2:sub(Vector3, Vector2));
		true ->
			absSq(rvo2_vector2:sub(Vector3, rvo2_vector2:add(Vector1, rvo2_vector2:mult(rvo2_vector2:sub(Vector2, Vector1), R))))
	end.
...
leftOf(A, B, C) ->
	det(rvo2_vector2:sub(A, C), rvo2_vector2:sub(B, A)).
